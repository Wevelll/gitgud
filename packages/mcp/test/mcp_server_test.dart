...
Map<String, Object?> req(Object? id, String method,
...
void main() {
  test('initialize echoes protocol version and advertises tools', () async {
    final r = await server()
        .handle(req(1, 'initialize', {'protocolVersion': '2025-06-18'}));
  ...
  test('tools/list returns all tools with schemas and destructive hints',
      () async {
    final r = await server().handle(req(2, 'tools/list'));
    final tools =
        ((r!['result'] as Map)['tools'] as List).cast<Map<String, Object?>>();
  ...
  test('tools/call runs a read tool and returns structured content', () async {
    final r = await server()
        .handle(req(3, 'tools/call', {'name': 'get_current_block'}));
    final result = r!['result'] as Map;
  ...
  test('denied consent surfaces as an isError tool result, not a crash',
      () async {
    final r = await server(consent: const DenyAllConsent())
        .handle(req(4, 'tools/call', {
      'name': 'add_block',
  ...
  test('domain errors become isError results', () async {
    final r = await server().handle(req(5, 'tools/call', {
      'name': 'delete_block',
      'arguments': {'id': 'nope'}
    }));
